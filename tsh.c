...
void eval(char *cmdline) {
  ...
  sigset_t x;
  if (sigemptyset(&x) == -1) unix_error("sigemptyset");
  if (sigaddset(&x, SIGCHLD) == -1) unix_error("sigaddset");
  if (sigprocmask(SIG_BLOCK, &x, NULL) == -1) unix_error("sigprocmask");

  pid_t pid = fork();
  if (pid == -1) unix_error("fork");

  if (pid == 0) {
    // child

    // child process inherit signal blocked vector, undo it
    if (sigprocmask(SIG_UNBLOCK, &x, NULL) == -1) unix_error("sigprocmask");

    // child process should be in a new process group
    if (setpgid(0, 0) == -1) unix_error("setpgid");

    if (execv(argv[0], argv) == -1) {
      printf("%s: Command not found\n", argv[0]);
      fflush(stdout); // _Exit won't flush io, must do it manually
      _Exit(1);       // use _Exit instead of exit in child proc
    }
...
void do_bgfg(char **argv) {
  ...
  if (argv[1][0] == '%') {
    ...
    if ((job = getjobpid(jobs, jobid)) == NULL) {
  ...
  if (strcmp(argv[0], "bg") == 0) {
    kill(-(job->pid), SIGCONT); // send signal to the whole process group
    job->state = BG;
...
void sigchld_handler(int sig) {
  ...
  if (WIFSIGNALED(wstatus)) { // terminated by signal
    ...
    fgjob->state = ST;
  } else if (WIFEXITED(wstatus)) { // normal exit
    deletejob(jobs, pid);
  }
}
...
void sigint_handler(int sig) {
  ...
  // SIGINT should be sent to the whole process group
  kill(-fg, SIGINT);
}
